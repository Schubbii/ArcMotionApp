...
/** Dependency-light SVG line chart with an area fill, themed via tokens. */
export function LineChart({ points, height = 200 }: Props) {
  ...
  const data = useMemo(() => {
    ...
    const sy = (y: number) => H - padY - ((y - lowY) / (highY - lowY)) * (H - padY * 2);
    ...
    const areaPath = `${linePath} L ${coords[coords.length - 1].x} ${H - padY} L ${
      single ? padX : coords[0].x
    } ${H - padY} Z`;
    ...
    return { coords, linePath, areaPath, gridLines };
  }, [points, H]);
  ...
  return (
    <Svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`}>
      ...
      <Path d={data.linePath} fill="none" stroke={t.primary} strokeWidth={2.5} />
      {data.coords.map((c, i) => (
        <Circle
          key={i}
          cx={c.x}
          cy={c.y}
          r={i === data.coords.length - 1 ? 4.5 : 3}
          fill={t.surface}
          stroke={t.primary}
          strokeWidth={2.5}
        />
